...
const HEADERS = ['Type', 'Term', 'IPA', 'Audio', 'Meaning', 'Example', 'Note', 'Tags', 'Added'];
...
function playSelected() {
  ...
  if (row < 2) { SpreadsheetApp.getActive().toast('Click a word row first.'); return; }

  const term = String(sh.getRange(row, 2).getValue() || '').trim();
  if (!term) { SpreadsheetApp.getActive().toast('That row has no word in it.'); return; }
  const ipa = String(sh.getRange(row, 3).getValue() || '');
  const url = audioFrom_(sh.getRange(row, 4).getFormula(), sh.getRange(row, 4).getValue());

  const html = HtmlService.createHtmlOutput(playerHtml_([{ term: term, ipa: ipa, audio: url }], true))
...
function openPlayer() {
  ...
  if (last >= 2) {
    ...
    const forms = sh.getRange(2, 4, last - 1, 1).getFormulas();
    items = vals.map(function (r, i) {
      return { term: String(r[1] || '').trim(), ipa: String(r[2] || ''), audio: audioFrom_(forms[i][0], r[3]) };
    }).filter(function (x) { return x.term; }).reverse().slice(0, 200);
  }
  const html = HtmlService.createHtmlOutput(playerHtml_(items, false)).setTitle('Pronunciation');
...
function playerHtml_(items, autoplay) {
  ...
  return '' +
    ...
    '<script>' +
    'var ITEMS=' + data + ',AUTO=' + (autoplay ? 'true' : 'false') + ';' +
...
/**
 * Rewrites Audio cells written before the scheme fix, and drops links that
 * were never valid. Run once after updating this script.
 */
function fixAudioLinks() {
  const sh = sheet_();
  const last = sh.getLastRow();
  if (last < 2) return;
  const rng = sh.getRange(2, 4, last - 1, 1);
  const forms = rng.getFormulas();
  const vals = rng.getValues();
  let fixed = 0, cleared = 0;

  for (let i = 0; i < vals.length; i++) {
    const raw = forms[i][0] || vals[i][0];
    if (!raw) continue;
    const url = audioFrom_(forms[i][0], vals[i][0]);
    const cell = sh.getRange(i + 2, 4);
    if (!url) { cell.clearContent(); cleared++; continue; }
    const current = String(forms[i][0] || '');
    if (current.indexOf(url) === -1) { cell.setFormula('=HYPERLINK("' + url + '","play")'); fixed++; }
  }
  SpreadsheetApp.getActive().toast('Repaired ' + fixed + ' link' + (fixed === 1 ? '' : 's') +
    (cleared ? ', cleared ' + cleared + ' broken one' + (cleared === 1 ? '' : 's') : '') +
    '. Use English > Play the word in this row to hear it.', 'English', 8);
}
...
function setUpSheet() {
  ...
  sh.getRange(1, 1, 1, HEADERS.length).setValues([HEADERS])
    .setFontWeight('bold').setBackground('#121B24').setFontColor('#FFFFFF');
  sh.setFrozenRows(1);
  sh.setColumnWidth(1, 90);   // Type
  sh.setColumnWidth(2, 170);  // Term
  sh.setColumnWidth(3, 140);  // IPA
  sh.setColumnWidth(4, 60);   // Audio
  sh.setColumnWidth(5, 360);  // Meaning
  sh.setColumnWidth(6, 360);  // Example
  sh.setColumnWidth(7, 220);  // Note
  sh.setColumnWidth(8, 140);  // Tags

  const rule = SpreadsheetApp.newDataValidation()
    .requireValueInList(TYPES, true).setAllowInvalid(false).build();
  sh.getRange(2, 1, sh.getMaxRows() - 1).setDataValidation(rule);

  SpreadsheetApp.getActive().toast('Sheet ready. Put the word in column B and pick a type in column A.');
}
...
function enrichAll() { run_(false); }
function enrichSelection() { run_(true); }
...
function run_(selectionOnly) {
  ...
  rows.forEach(function (row, i) {
    ...
    if (!term) return;
    if (!row[8]) row[8] = new Date();               // Added
    if (!type) row[0] = guessType_(term);           // default Type
    if (!shouldLookUp_(String(row[0]).toLowerCase(), term)) return;  // phrases aren't in the dictionary

    const hasAll = row[2] && row[4];
    if (hasAll && !selectionOnly) return;           // already done

    const data = lookUp_(term);
    if (!data) { missed.push(term); return; }

    if (data.ipa) row[2] = data.ipa;
    if (data.audio) row[3] = '=HYPERLINK("' + data.audio + '","play")';
    if (data.meaning) row[4] = data.meaning;
    if (FILL_EXAMPLE_IF_BLANK && !row[5] && data.example) row[5] = data.example;
    filled++;
    Utilities.sleep(150);
  });
  ...
  if (missed.length) msg += ' Not in the dictionary: ' + missed.slice(0, 6).join(', ') +
    (missed.length > 6 ? ' and ' + (missed.length - 6) + ' more' : '') + '. Write those by hand.';
  SpreadsheetApp.getActive().toast(msg, 'English', 8);
}

/**
 * Fires when you finish typing in column B. Installable trigger only —
...
function onEditInstalled(e) {
  ...
  if (!sh.getRange(row, 1).getValue()) sh.getRange(row, 1).setValue(guessType_(term));
  if (!sh.getRange(row, 9).getValue()) sh.getRange(row, 9).setValue(new Date());
  if (!shouldLookUp_(String(sh.getRange(row, 1).getValue()).toLowerCase(), term)) return;

  const data = lookUp_(term);
  if (!data) { sh.getRange(row, 7).setValue('Not in dictionary — write the meaning yourself'); return; }
  if (data.ipa) sh.getRange(row, 3).setValue(data.ipa);
  if (data.audio) sh.getRange(row, 4).setFormula('=HYPERLINK("' + data.audio + '","play")');
  if (data.meaning) sh.getRange(row, 5).setValue(data.meaning);
  if (FILL_EXAMPLE_IF_BLANK && !sh.getRange(row, 6).getValue() && data.example) {
    sh.getRange(row, 6).setValue(data.example);
  }
}
...
function lookUp_(term) {
  ...
  const out = { ipa: '', audio: '', meaning: '', example: '' };
...
function sheet_() {
  ...
  if (!sh) throw new Error('No sheet named "' + SHEET_NAME + '". Run Set up sheet first.');
  return sh;
}
 ...
 *   { key, type, term, example, note, tags, source }
 * Fills in IPA / audio / meaning on the way in, and merges instead of
...
function doPost(e) {
  ...
  if (!authorised_(body.key)) return json_({ ok: false, error: 'Wrong or missing save key' });

  const term = String(body.term || '').trim().replace(/\s+/g, ' ');
  if (!term) return json_({ ok: false, error: 'No term to save' });
  if (term.length > 500) return json_({ ok: false, error: 'That is too long to be one entry' });

  const type = TYPES.indexOf(String(body.type || '')) >= 0 ? String(body.type) : guessType_(term);
  ...
  try {
    ...
    let looked = { ipa: '', audio: '', meaning: '', example: '' };
    if (shouldLookUp_(type, term)) looked = lookUp_(term) || looked;

    if (existing) {
      // Already there — top up the blanks rather than making a second row.
      const r = existing;
      if (!sh.getRange(r, 3).getValue() && looked.ipa) sh.getRange(r, 3).setValue(looked.ipa);
      if (!sh.getRange(r, 4).getValue() && looked.audio) {
        sh.getRange(r, 4).setFormula('=HYPERLINK("' + looked.audio + '","play")');
      }
      if (!sh.getRange(r, 5).getValue() && looked.meaning) sh.getRange(r, 5).setValue(looked.meaning);
      if (example) appendCell_(sh.getRange(r, 6), example);
      if (note) appendCell_(sh.getRange(r, 7), note);
      if (tags) appendCell_(sh.getRange(r, 8), tags, ', ');
      return json_({
        ok: true, action: 'updated', row: r, type: type, term: term,
        ipa: sh.getRange(r, 3).getValue(), meaning: sh.getRange(r, 5).getValue()
      });
    }
    ...
    const row = [
      ...
      '',
      looked.meaning || '',
      example || (FILL_EXAMPLE_IF_BLANK ? looked.example : '') || '',
      note,
      tags,
      new Date()
    ];
    ...
    if (looked.audio) sh.getRange(r, 4).setFormula('=HYPERLINK("' + looked.audio + '","play")');

    return json_({
      ok: true, action: 'added', row: r, type: type, term: term,
      ipa: looked.ipa || '', meaning: looked.meaning || ''
    });
  } catch (err) {
...
function findRow_(sh, term, type) {
  ...
  for (let i = 0; i < vals.length; i++) {
    if (String(vals[i][1] || '').trim().toLowerCase() === needle &&
        String(vals[i][0] || '').trim().toLowerCase() === type) return i + 2;
  }
  return 0;
}
...
function appendCell_(cell, text, joiner) {
  ...
  if (!current) { cell.setValue(text); return; }
...
function setSaveKey() {
  ...
  const res = ui.prompt('Save key',
    'Pick a long random string. Paste the same one into the extension popup.\n' +
    'Leave blank to remove the key.', ui.ButtonSet.OK_CANCEL);
  if (res.getSelectedButton() !== ui.Button.OK) return;
  const key = res.getResponseText().trim();
  const props = PropertiesService.getScriptProperties();
  if (key) { props.setProperty('API_KEY', key); ui.alert('Saved. Put this in the extension popup:\n\n' + key); }
  else { props.deleteProperty('API_KEY'); ui.alert('Key removed — anyone with the URL can post rows.'); }
}
...
function doGet(e) {
  ...
  try {
    ...
    if (last >= 2) {
      ...
      const formulas = sh.getRange(2, 4, last - 1, 1).getFormulas();

      items = values.map(function (r, i) {
        return {
          type: String(r[0] || 'vocab').toLowerCase().trim(),
          term: String(r[1] || '').trim(),
          ipa: String(r[2] || '').trim(),
          audio: audioFrom_(formulas[i][0], r[3]),
          meaning: String(r[4] || '').trim(),
          example: String(r[5] || '').trim(),
          note: String(r[6] || '').trim(),
          tags: String(r[7] || '').split(',').map(function (s) { return s.trim(); }).filter(String),
          added: r[8] ? new Date(r[8]).toISOString().slice(0, 10) : ''
        };
  ...
  return ContentService
    .createTextOutput(JSON.stringify({ ok: !error, error: error, count: items.length, items: items }))
    .setMimeType(ContentService.MimeType.JSON);
}
